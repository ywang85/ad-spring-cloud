...
public class AdUnitServiceImpl implements IAdUnitService {
    ...
    public AdUnitResponse createUnit(AdUnitRequest request) throws AdException {
        ...
        Optional<AdPlan> plan = adPlanRepository.findById(request.getPlanId());
        if (!plan.isPresent()) {
    ...
    public AdUnitKeywordResponse createUnitKeyword(AdUnitKeywordRequest request) throws AdException {
        ...
        if (!CollectionUtils.isEmpty(request.getUnitKeywords())) {
            // 批量创建AdUnitKeyword对象，并且存入数据库，返回所有的id
            request.getUnitKeywords().forEach(i -> unitKeywords.add(new AdUnitKeyword(i.getUnitId(), i.getKeyword())));
            ids = adUnitKeywordRepository.saveAll(unitKeywords).stream().map(AdUnitKeyword::getId)
                    .collect(Collectors.toList());
        }
    ...
    public AdUnitDistrictResponse createUnitDistrict(AdUnitDistrictRequest request) throws AdException {
        ...
        List<Long> ids = adUnitDistrictRepository.saveAll(adUnitDistricts).stream().map(AdUnitDistrict::getId)
                .collect(Collectors.toList());
